...
export default function Settings() {
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex items-center gap-1 bg-zinc-100 dark:bg-surface-subtle/50 p-1.5 rounded-2xl w-fit border border-zinc-200 dark:border-surface-border shadow-inner">
        {[
          ...
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${
              activeTab === tab.id
                ? 'bg-white dark:bg-surface-card text-zinc-900 dark:text-white shadow-lg ring-1 ring-zinc-200 dark:ring-surface-border'
        ...
        {activeTab === 'channels' && (
          <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
              ...
              >
                <Plus className="w-5 h-5 mr-2" /> Vincular Canal
              </button>
            </div>
            ...
            {loading.channels && channels.length === 0 ? (
              ...
              <div className="p-20 border-2 border-dashed border-zinc-200 dark:border-surface-border rounded-3xl flex flex-col items-center text-center bg-zinc-50/30 dark:bg-surface-subtle/10">
                <div className="w-16 h-16 rounded-2xl bg-white dark:bg-surface-card flex items-center justify-center mb-6 shadow-xl shadow-zinc-200 dark:shadow-none ring-1 ring-zinc-200 dark:ring-surface-border">
      ...
      <Modal isOpen={createOpen} onClose={() => { setCreateOpen(false); setEditTarget(null); }} title={editTarget ? 'Editar Canal' : 'Conectar Novo Canal'}>
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1"><label className={labelCls}>Nome da Instância</label><input placeholder="Ex: Recepção" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} /></div>
            <div className="space-y-1">
              <label className={labelCls}>Provedor Meta/WhatsApp</label>
                ...
                >
                  <option value="META" className="bg-white dark:bg-surface-card">Meta Official API</option>
                  <option value="GUPSHUP" className="bg-white dark:bg-surface-card">Gupshup Hub</option>
                </select>
            </div>
          </div>
          ...
          <div className="animate-in slide-in-from-right-4 duration-300">
               <>
                  ...
                  <div className="p-4 bg-zinc-50 dark:bg-surface-subtle/50 border border-zinc-200 dark:border-surface-border rounded-xl space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-6 border-t border-zinc-100 dark:border-surface-border/50">
            ...
            <button onClick={handleSave} disabled={!form.name} className="btn-primary px-8">
      ...
      <Modal isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Desvincular Canal" size="sm">
        ...
        <div className="flex justify-end gap-3 mt-8 pt-6 border-t border-zinc-100 dark:border-surface-border/50">
          <button onClick={() => setDeleteTarget(null)} className="btn-ghost">Cancelar</button>
          <button onClick={handleDelete} className="btn-danger px-8">Remover Definitivamente</button>
        </div>
      </Modal>
